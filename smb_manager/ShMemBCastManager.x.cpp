...
int generateDefaultLogFilePath(char* path, size_t size, std::string vlan) {
  ...
  size_t bytesWritten =
      ::snprintf(path,
                 size,
                 ShMemBCastProtocol::MANAGER_VLAN_LOCATION_FORMAT,
                 vlan.c_str());
  if (('\0' != path[size - 1]) ||
...
int parseUserValue(std::set<uid_t>* permittedUIDSet, const std::string& value) {
  ...
  const char* parseEnd;
  uid_t uid = StrToInt::parseUint(value.c_str(),
                                        StrToInt::npos,
                                        10,
                                        &parseEnd);
  ...
  if (0 == userDetails) {
    std::cerr << "Could not find User Details for username \""
              << value
              << "\""
              << std::endl;
    return -1;
 ...
 * @return 0 on success, non-zero on error. Prints error messages to stderr
 */
int parseGroupValue(std::set<gid_t>* permittedGIDSet, const std::string& value) {
  // check if it is a gid
  const char* parseEnd;
  gid_t gid = StrToInt::parseUint(value.c_str(),
                                        StrToInt::npos,
                                        10,
                                        &parseEnd);
  if (parseEnd == (value.c_str() + value.size())) {
  ...
  if (0 == groupDetails) {
    std::cerr << "Could not find Group Details for groupname \""
              << value
              << "\""
              << std::endl;
    return -1;
...
int parsePermissions(std::set<uid_t>* permittedUIDSet,
  ...
  typedef boost::token_iterator_generator<
    boost::char_separator<char> >::type TokenIterator;

  // tokenize the permissions
  boost::char_separator<char> separator(",");
  TokenIterator iter =
      boost::make_token_iterator<std::string>(permissions.begin(),
                                              permissions.end(),
                                              separator);
  TokenIterator iterEnd =
      boost::make_token_iterator<std::string>(permissions.end(),
                                              permissions.end(),
                                              separator);
...
void usage(const char* programName) {
  std::cerr << programName << " [option]*"
            ...
            << "(default: 4MiB, constraints: > 64KiB)"
            << std::endl
...
int main(int argc, char** argv) {
  ...
  while (-1 != (option = ::getopt_long(argc, argv, optstring, longopts, 0))) {
    switch (option) {
      ...
      case 'b': {
        ...
        if (0 == bufferSize) {
          std::cerr << "Please enter a valid buffer size in bytes"
                    << std::endl;
          usage(argv[0]);
          return 1;
        }
      ...
      case ':': {
        ...
        switch (optopt) {
          case 'v': {
            std::cerr << "Please specify the vlan to use"
                      << std::endl;
            usage(argv[0]);
            return 1;
          } break;
          ...
          case 'p': {
            std::cerr << "Please specify access permissions"
                      << std::endl;
            usage(argv[0]);
          } break;
          ...
          case 'l': {
            std::cerr << "Please specify a log file"
                      << std::endl;
            usage(argv[0]);
            return 1;
          } break;
  ...
  if (daemon &&
    ...
    std::cerr << "Log File cannot be stdout when run as a daemon"
              << std::endl;
    usage(argv[0]);
    return 1;
  }
  ...
  if (bufferSize < MINIMUM_BUFFER_SIZE) {
    std::cerr << "Default buffer Size must be at least "
              << MINIMUM_BUFFER_SIZE
              << " bytes to fit at least one datagram!"
              << std::endl;
    return 1;
  }
  ...
  if (logFilePath.empty()) {
    ...
    if (0 != generateDefaultLogFilePath(path, sizeof(path), vlan)) {
      std::cerr << "Could not generate default log file path"
                << std::endl;
      usage(argv[0]);
      return 1;
    }
  ...
  if (0 != manager.init(vlan,
                        permittedUIDSet,
                        permittedGIDSet,
                        bufferSize,
                        logFilePath)) {
    std::cerr << "Could not initialize Manager"
              << std::endl;
    return 1;
  }
